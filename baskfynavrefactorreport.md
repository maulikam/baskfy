...
### F11 — Fragmented search — **CLOSED 25 Aug 2026 (M46)**
...
page, so an index hit lands on `/market/today?q=<slug>` — see `docs/DECISIONS-MERGE.md` §M46.2.
...
**Global search (F11)** — ✅ shipped 25 Aug 2026 (M46)
...
5. ~~**Global ⌘K search** across stocks/indices/baskets/screens (F11).~~ **Shipped 25 Aug 2026**
   (M46). One federated `GET /api/v1/search`; the palette draws Stocks · Indices · Baskets ·
   Screens · Go to from a single round trip, with recent items. Six Playwright tests in
   `apps/web/e2e/search.spec.ts`. Decisions: `docs/DECISIONS-MERGE.md` §M46–M46.7.
