...
export const metadata: Metadata = {
  title: PAGES["/me/investments"].title,
  description: PAGES["/me/investments"].blurb,
...
export default async function InvestmentsPage() {
  ...
  return (
    <div className="flex max-w-3xl flex-col gap-6">
      ...
      <PageHeader title={PAGES["/me/investments"].title} blurb={PAGES["/me/investments"].blurb} />
      ...
      {active.length === 0 ? (
        <div className="grid place-items-center rounded-xl border border-dashed border-border bg-card/50 px-6 py-16 text-center">
          <p className="max-w-[52ch] text-sm leading-relaxed text-muted-foreground">
            No basket investments yet. Browse the catalog, then Invest — that builds an order plan;
