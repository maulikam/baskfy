...
/**
 * "Scan now" on `/twt` — leaf 5 of `PLAN-SCAN-SYNC.md`.
 *
 * **Built to the contract, not to a running route.** `POST /twt/scan` answers 202 queued, 409 one
 * already in flight, 429 one a minute; `GET /twt/scan/{run_id}` carries the state. Leaf 4 builds
...
function answering(result: TwtScanResult) {
  return vi.fn((): Promise<TwtScanResult> => Promise.resolve(result));
}

async function press(): Promise<void> {
  const form = screen.getByTestId("twt-scan-now");
...
function statusText(): string {
  return screen.getByTestId("twt-scan-status").textContent ?? "";
}
...
describe("accessible: the control is a real button and announces itself", () => {
  it("accessible: renders a submit button inside a real form, enabled and focusable", () => {
    ...
    const button = within(screen.getByTestId("twt-scan-now")).getByRole("button", {
      name: "Scan now",
    });
  ...
  it("accessible: one polite live region, present before anything has happened", () => {
    ...
    expect(regions[0]).toBe(screen.getByTestId("twt-scan-status"));
  });
  ...
  it("accessible: a finished run is not announced as a failure", () => {
    ...
    expect(screen.getByTestId("twt-scan-status").className).toContain("text-muted-foreground");
  ...
  it("accessible: a failed run is coloured with the semantic token, not a palette step", () => {
    ...
    expect(screen.getByTestId("twt-scan-status").className).toContain("text-negative");
  });
});
...
/**
 * The scan `components/twt/__tests__/no-internals.test.tsx` already runs over the rest of `/twt`,
 * applied here to every state this control can be in. A route, a column, a file, a host or a setting name on a
