...
/**
 * The one write `/twt` has — "Scan now", and nothing else, ever.
 *
 ...
 * was making: `docs/twt/02` Track C §4 gives the web app no route under `/twt` that can reach the
 * gateway, and `05` §2 still says a plan line becomes an order in the desk console, on a click a
 * person makes, and nowhere else. **Queuing a detector is not that.** A scan reads prices and
 ...
 *    `/twt/scan` would admit the desk's confirm route as well, and that route is exactly what
 ...
 *
 * The status is returned rather than the server's sentence, on purpose — see `../../app/(app)/twt/copy.ts`.
 */

/** The only path this application may write to under `/twt`. Widening it is a deliberate act. */
export type TwtWritePath = "/twt/scan";

/** What a form gets back. The sentence is chosen by this app's copy, never by the wire. */
export type TwtScanResult =
...
export async function twtWrite(path: TwtWritePath): Promise<TwtWriteOutcome> {
