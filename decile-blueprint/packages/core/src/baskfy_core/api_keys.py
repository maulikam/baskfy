...
what a key is allowed to mean; ``baskfy_api.api_keys`` mints, stores and verifies them, and
``baskfy_api.routers.api_keys`` exposes the lifecycle.
...
* ``secret`` is 256 bits, URL-safe, and is **never stored**. ``baskfy_api.security.digest``
  ...
  matters is that a database dump cannot be replayed. See ``baskfy_api.security``'s module
  docstring.
...
#: as a refresh token (``baskfy_api.security.OPAQUE_TOKEN_BYTES``).
...
def redact(presented: str) -> str:
    ...
    ``baskfy_api.logging``'s redaction filter is the backstop; this is the thing that means the
