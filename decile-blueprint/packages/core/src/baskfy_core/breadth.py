...
Pure, in the same sense ``baskfy_core.screener`` is pure: it builds a SQLAlchemy ``Select`` and
touches nothing. The nightly step (``baskfy_worker.tasks.market_health``) executes it and stores
...
from baskfy_core.models import FactorDaily, IndexMemberDaily
