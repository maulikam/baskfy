...
MOVED TO CORE AT M15 (P3.3), unchanged.
It was already pure — no configuration, no I/O, no clock — so nothing had to change to
fit core's first law. The statutory rates are the model, and they are constants because
regulation sets them. `kite_momentum_rebalancer.app.costs` is now a re-export.
"""
...
class Cost:
    ...
    def total(self) -> float:
        return (
            self.stt + self.exchange + self.sebi + self.stamp + self.gst + self.dp + self.brokerage
        )
...
def plan_cost(orders: Iterable[Mapping[str, float]]) -> dict[str, float]:
