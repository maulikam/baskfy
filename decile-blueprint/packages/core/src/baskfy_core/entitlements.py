...
This module is the **single definition** of that shape. It lives in `baskfy_core` — which is
I/O-free — because three places need it and a second copy would be a second thing to keep in
step:

* ``baskfy_core.seed_data`` writes it into ``plan.features`` (docs/04's ``features jsonb``),
* ``baskfy_api.entitlements`` resolves it for a caller and renders it into ``GET /me``,
...
:class:`FeatureNotEntitled`, which ``baskfy_api.app`` renders as docs/07's ``402
...
class Override:
    ...
    A plain value object rather than the ORM row (``baskfy_core.models.admin.EntitlementOverride``)
