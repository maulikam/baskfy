...
yields no signing key. See ``baskfy_api.webhooks``.
"""
...
class ApiKey(Base):
    ...
    window)" — actually requires. See ``baskfy_api.api_keys.authenticate``.
    """
    ...
    #: ``baskfy_core.api_keys.Scope`` values. Every one of them is a read.
    ...
    #: Advanced on use. Approximate by design — see ``baskfy_api.api_keys`` for the write policy.
...
class ScreenAlert(Base):
    ...
    (``baskfy_api.security``). That is what lets one-click unsubscribe work without a session.
    ...
    #: ``baskfy_core.screen_diff.diff_runs``.
...
class WebhookDelivery(Base):
    ...
    and ``next_attempt_at`` are what the backoff schedule in ``baskfy_api.webhooks`` computes
