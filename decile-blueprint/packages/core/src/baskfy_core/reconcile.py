...
This module is the arithmetic; ``baskfy_worker.reconcile_cli`` is the command that prints it.
...
``BASKFY_PARITY_BARS`` path in ``packages/core/tests/test_reference_parity.py``, and it skips.
...
def check_skip_month_momentum(frame: pl.DataFrame) -> CheckResult:
    ...
    * **The export has no skip-month column.** All 93 are listed in
      ``baskfy_core.reference_export.EXPORT_COLUMNS``; none of them is ``ret_12m_minus_1m`` or
...
def skip_month_evidence(frame: pl.DataFrame) -> list[str]:
    ...
    lines = [
        ...
        "remains a constructor argument (baskfy_core.momentum.SkipMonthConfig). Resolving it "
...
def check_circuit_detection_rule(frame: pl.DataFrame) -> CheckResult:
    ...
    return CheckResult(
        ...
        unresolved=[
            ...
            "(baskfy_core.circuits.CircuitMethod). Recalibration needs bhavcopy history.",
        ],
    )
