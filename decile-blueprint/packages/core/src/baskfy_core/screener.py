...
This module turns a validated :class:`~baskfy_core.screen_definition.ScreenDefinition` plus an
...
I/O-free, so execution and caching live in ``baskfy_api.screener``.
...
are the ``sql_expr`` values written by hand in ``baskfy_core.factor_registry``.
...
#: The prefix is shared with ``baskfy_worker.tasks.publish``, which purges by it.
...
def _factor_expression(key: str) -> ColumnElement[object]:
    ...
    came out of ``baskfy_core.factor_registry``, never off the wire, and :func:`validate_definition`
...
#: (``baskfy_core.reference_export.UNMAPPED_EXPORT_COLUMNS``).
