...
factor math unit-testable and the backtest engine reusable." ``decile_worker.backtest`` loads the
...
2. Target = the top ``top_n``, widened by the **hold buffer**: a name already held is retained
   ...
   :func:`decile_core.rebalance.plan_rebalance`, the same function the rebalance tracker calls,
...
in :mod:`decile_core.backtest_metrics` and are computed in NumPy. A Sharpe ratio expressed in
...
from decile_core.rebalance import HeldName, ScreenRank, plan_rebalance
...
def _decide(  # noqa: PLR0913, PLR0917 - a decision joins the screen, the book and the overlay
    ...
    if config.risk_overlay.enabled and not _risk_on(reader, day, benchmark_levels, notes):
        ...
        # The rank-buffer rule is `decile_core.rebalance`, not a second copy of it: the tracker
