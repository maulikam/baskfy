...
* ``baskfy_refresh`` — httpOnly, ``SameSite=Lax``, Secure. The credential. JavaScript cannot read
  it, which is what makes it worth stealing and what makes it un-stealable by XSS.
* ``baskfy_csrf`` — **not** httpOnly, same lifetime. A random value with no authority of its own.
...
REFRESH_COOKIE: Final = "baskfy_refresh"

#: The readable half of the double submit.
CSRF_COOKIE: Final = "baskfy_csrf"
