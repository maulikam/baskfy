...
* **An unpaid account's ``max_screens`` is 5, not 50.** docs/07's example payload shows 50 for a
  subscriber and the bundle gives no free-tier number; see ``baskfy_core.entitlements``.

The shape of the answer is docs/07's, exactly seven keys. ``Feature`` and ``Entitlements`` are
defined in ``baskfy_core.entitlements`` so the seed data, this service and ``GET /plans`` share
...
from baskfy_api.auth import Principal, PrincipalDep, settings_for
...
from baskfy_core.models import EntitlementOverride, Plan, Subscription
from baskfy_core.seed_data import FREE_PLAN
