...
The Redis half and the warm-up already exist (``baskfy_api.screener``,
``baskfy_worker.tasks.publish``). This module is the HTTP half: a validator on every published
...
``private``, always. These responses are entitlement-filtered (``baskfy_api.entitlements``) and a
...
``StreamingResponse`` whose whole point is that it is never buffered (``baskfy_api.csv_export``),
...
def snapshot_headers(as_of: dt.date | None, data_version: int | None) -> dict[str, str]:
    ...
    ``POST /screens/{id}/run`` has set them since Prompt 7 and ``baskfy_api.app`` already lists
...
def register_http_cache(app: FastAPI, settings: Settings) -> None:
    ...
    Registered from :func:`baskfy_api.app.register_middleware` so its position relative to the
