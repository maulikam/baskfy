...
Where the PDF goes
------------------
``baskfy_providers.archive`` already models "an S3-compatible bucket, or a directory when there is
...
from baskfy_core.gst import GstSplit, financial_year, split_inclusive
...
def build_invoice_archive(settings: Settings) -> RawArchive:
    ...
    ``baskfy_providers.factory.build_archive`` already makes exactly that choice for the raw-file
