...
The algorithm is the token bucket ``baskfy_providers.ratelimit`` already runs for Kite — same Lua
...
from baskfy_api.auth import Principal, PrincipalDep, PrincipalKind
from baskfy_api.problems import Problem, ProblemType, rate_limited
from baskfy_api.settings import Settings
from baskfy_providers.ratelimit import TAKE_SCRIPT
...
class RateLimiter:
    ...
    async def check_key(self, prefix: str, limit_per_minute: int) -> Decision:
        ...
        stored unhashed and shown in the UI (``baskfy_core.api_keys``).
        """
...
async def enforce_rate_limit(request: Request, principal: PrincipalDep) -> None:
    """The dependency every ``/api/v1`` route carries (see ``baskfy_api.app``).
