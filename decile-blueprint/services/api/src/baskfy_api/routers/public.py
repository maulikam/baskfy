...
1. ``BASKFY_PUBLIC_API_ENABLED`` — a setting, ``False`` in every environment file in this
   repository, asserted by ``services/api/tests/test_public_api.py``.
2. ``baskfy_core.public_api.DATA_REDISTRIBUTION_REVIEW.signed_off`` — a **source constant**, and
...
:func:`is_enabled` is the single place both are read, and ``baskfy_api.app`` only mounts this
...
-------------------------------
``baskfy_core.public_api.PUBLIC_COLUMNS`` is the whitelist, and the rule that produces it is "no
...
from baskfy_api import api_keys as key_service
from baskfy_api import market_data, public_docs
from baskfy_api.auth import API_KEY_HEADER
from baskfy_api.db import SessionDep
from baskfy_api.problems import Problem, ProblemType, not_found, rate_limited, unauthenticated
from baskfy_api.ratelimit import RateLimiter
...
from baskfy_core.models import ApiKey, FactorDaily, Instrument, Screen
...
from baskfy_core.screener import ScreenResult
...
async def authenticated_key(request: Request, session: SessionDep) -> ApiKey:
    ...
    ``baskfy_api.api_keys``, and Prompt 20's first acceptance criterion.
...
async def public_reference(request: Request) -> Response:
    """Redoc, pointed at the document above. See ``baskfy_api.public_docs``."""
    settings = _settings(request)
    return Response(
