...
and ``baskfy_api.ratelimit.is_webhook_path`` gives anything under it a 600/min per-IP bucket sized
...
its owner, because it is derived rather than stored — ``baskfy_api.webhooks``. That is a
...
from baskfy_api import webhooks as service
