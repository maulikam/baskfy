...
#: responses are serialised with `baskfy_core.screener.canonical_json`, the same encoder the
...
# ---------------------------------------------------------------------------
# Backtests — docs/07 §Backtests, docs/10 (Prompt 15)
# ---------------------------------------------------------------------------


class BacktestCreate(_In):
    """docs/07: `POST /backtests { config }`.
    ...
    #: docs/07 §"Running a screen" takes the same optional guard; a stale one is a 409.
    data_version: int | None = None
    #: Run docs/10's four perturbations alongside the base run. Costs four more simulations.
    fragility: bool = True


class BacktestAcceptedOut(_Out):
    """docs/07: `→ 202 { public_id, status:"queued" }`."""

    public_id: str
    status: Literal["queued", "running", "done", "failed"]
    created_at: dt.datetime
    #: Where the progress stream lives, so a client does not have to construct it.
    events_url: str


class BacktestSummaryOut(_Out):
    """One row of the list. Not in docs/07 — see `docs/DECISIONS.md` §15."""
...
class BacktestListOut(_Out):
    data: list[BacktestSummaryOut]


class EquityPointOut(_Out):
    """One point of the downsampled curve docs/10 keeps in Postgres."""
...
class DrawdownPointOut(_Out):
    date: dt.date
    drawdown: float


class MonthlyReturnOut(_Out):
    """One cell of docs/10 §Artefacts' "monthly return heatmap"."""
...
class FragilityRunOut(_Out):
    """docs/10 §"honesty features": one of the five runs in the fragility readout."""
...
class BacktestOut(_Out):
    ...
    #: The whole docs/10 §Outputs metric block, as stored. ``JsonValue`` rather than a model with
    #: twenty-eight optional floats: the block is generated by
    #: ``baskfy_core.backtest_metrics.Metrics.as_dict`` and duplicating its shape here would
    #: create two definitions of one payload to keep in step.
    ...
    monthly_returns: list[MonthlyReturnOut] = Field(default_factory=list)
    fragility: list[FragilityRunOut] = Field(default_factory=list)
    #: docs/10 §"honesty features" — the assumptions panel, in prose, from the server.
    assumptions: list[str] = Field(default_factory=list)
    disclaimer: str


class TradeOut(_Out):
    """docs/10 §Artefacts: "date, symbol, side, qty, price, cost, reason"."""
    ...
    notional: Decimal
    cost: Decimal
    reason: Literal["enter", "exit", "rebalance", "delist"]
    realised_pnl: Decimal | None = None


class TradePage(_Out):
...
class BacktestHoldingOut(_Out):
    """docs/10 §Artefacts: "per-rebalance holdings with weights"."""
...
class HoldingPage(_Out):
    data: list[BacktestHoldingOut]
    next_cursor: str | None = None


class ExportLinkOut(_Out):
    """docs/07: `GET /backtests/{id}/export` → "CSV/Parquet signed URL"."""

    artefact: Literal["trades", "holdings", "equity"]
    format: Literal["csv"]
    url: str
...
class PipelineStepOut(_Out):
    ...
    #: on a successful one — what makes a six-month-old number explicable (`baskfy_worker.steps`).
...
class ApiKeyCreate(_In):
    name: str = Field(min_length=1, max_length=120)
    #: Omitted means every scope — and every scope is a read (`baskfy_core.api_keys`).
...
class PublicTermsOut(_Out):
    """docs/11 §Compliance's constraint, machine-readable. See `baskfy_core.public_api`."""
...
class PublicScreenRowOut(_Out):
    ...
    `baskfy_core.public_api.PUBLIC_COLUMNS`, which the registry decides — declaring thirty
...
class PublicApiGateOut(_Out):
    ...
    #: `BASKFY_PUBLIC_API_ENABLED`. The operational switch.
    flag_enabled: bool
    #: `baskfy_core.public_api.DATA_REDISTRIBUTION_REVIEW.signed_off`. The compliance gate.
