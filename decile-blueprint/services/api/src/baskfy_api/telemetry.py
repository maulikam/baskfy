...
Off unless ``BASKFY_OTEL_ENABLED`` is set. Tracing every request on a laptop costs latency and
produces spans nobody reads, and a default-on exporter pointed at nothing logs a connection error
per span. Turning it on with no ``BASKFY_OTEL_EXPORTER_OTLP_ENDPOINT`` installs the tracer and no
...
``baskfy.``, because an attribute key invented at a call site is one a Grafana query will not find.
"""
...
ATTRIBUTES: Final[tuple[str, ...]] = (
    ...
    "baskfy.rows_in",
    "baskfy.rows_out",
    "baskfy.screen_definition_hash",
    "baskfy.request_id",
    "baskfy.pipeline_step",
    "baskfy.pipeline_run_id",
    "baskfy.step_status",
    "baskfy.as_of",
    "baskfy.data_version",
    "baskfy.cache_hit",
    "baskfy.problem_type",
    "baskfy.provider",
    "baskfy.retry_attempt",
    "baskfy.comstrument_id",
    "baskfy.backtest_id",
    "baskfy.alert",
)
...
def configure_telemetry(app: FastAPI, settings: Settings) -> None:
    ...
    # The API is a Celery *producer* (`baskfy_api.queue`). Instrumenting it is what writes the
...
def annotate_current_span(attributes: Mapping[str, AttributeValue]) -> None:
    ...
    (``baskfy.trade_date``) and therefore not a Python identifier — a ``**{...}`` splat at every
