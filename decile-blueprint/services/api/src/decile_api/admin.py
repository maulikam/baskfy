...
-----------------------------------
"Re-run" and "reprocess instrument" **enqueue** ``decile.pipeline.nightly`` and
``decile.compute.reprocess_instrument`` and answer 202. A nightly chain takes minutes and a
...
The same producer-by-task-name arrangement as ``POST /backtests`` (see ``decile_api.queue`` for
...
``decile_core.models.admin`` for why that table exists when nothing in the bundle asks for it.
"""
...
from decile_api.auth import Principal, PrincipalKind
from decile_api.entitlements import entitlements_for, resolve_active_grant
from decile_api.metrics import publish_latency_seconds
from decile_api.problems import Problem, ProblemType
from decile_api.queue import TaskQueue
from decile_api.settings import Settings
from decile_core.entitlements import MAX_SCREENS_KEY, Entitlements, Feature
...
#: for the reason ``decile_api.queue`` gives — ``decile-worker`` depends on ``decile-api``, so the
...
NIGHTLY_TASK_NAME: Final = "decile.pipeline.nightly"
REPROCESS_TASK_NAME: Final = "decile.compute.reprocess_instrument"
...
async def data_version_history(
    ...
    passed (``decile_worker.tasks.publish``), so "the history of data_version" and "the runs that
...
def provider_health() -> list[dict[str, object]]:
    ...
    Reuses ``decile_providers.cli.doctor_report`` rather than reimplementing the checks, so the
    ...
    on a page load.
    """
    from decile_providers.cli import doctor_report  # noqa: PLC0415
    from decile_providers.factory import build_provider_stack  # noqa: PLC0415
...
async def enqueue_rerun(
    ...
    """The "re-run" button. Publishes ``decile.pipeline.nightly`` for one date.
...
async def user_detail(
    ...
    Resolved through ``decile_api.entitlements.entitlements_for``, the same call every gated
