...
The arithmetic is ``decile_core.screen_diff`` and the wording is
``decile_api.email.templates.screen_alert``. This module is what sits between them: the
...
``decile-api`` (``decile_worker.tasks.celery_tasks`` imports the screener from it) and the
...
from decile_api.email import AlertRow, AlertSection, Mailer
from decile_api.email import templates as email_templates
from decile_api.screener import current_data_version
from decile_api.security import digest
...
from decile_core.screen_diff import RankedRow, ScreenDiff, diff_runs, rows_from_results
...
def unsubscribe_token(settings: Settings, public_id: str) -> str:
    ...
    if not master:
        raise RuntimeError(
            "No DECILE_WEBHOOK_SIGNING_SECRET or DECILE_JWT_SECRET, so an unsubscribe link "
...
async def ensure_run(session: AsyncSession, screen: Screen, as_of: dt.date) -> ScreenRun | None:
    ...
    runs a screen (``decile_api.routers.screens``); the nightly publish warms the Redis cache but
    ...
    It writes through the same upsert the API route uses (``decile_api.screener.record_run``), so
    ...
    """
    from decile_api.screener import execute_screen, record_run  # noqa: PLC0415 - avoids a cycle
...
async def dispatch(  # noqa: PLR0912, PLR0915 - one linear pass over the night's work
    ...
    *,
    ...
    """
    from decile_api import webhooks  # noqa: PLC0415 - avoids a cycle at import time
...
async def _recipient_addresses(session: AsyncSession, user_ids: Sequence[int]) -> dict[int, str]:
    ...
    from decile_core.models import AppUser  # noqa: PLC0415 - local to keep the import list short
