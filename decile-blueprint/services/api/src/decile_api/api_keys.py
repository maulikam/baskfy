...
The shape of a key is ``decile_core.api_keys`` (pure); this module is everything that touches the
database. ``decile_api.routers.api_keys`` is the lifecycle over HTTP and
``decile_api.routers.public`` is the only place a key is accepted as a credential.
...
from decile_api.security import digest, digests_match
