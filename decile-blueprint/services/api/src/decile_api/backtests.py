...
:class:`decile_core.backtest.BacktestResult` and the ``backtest`` row docs/04 defines.
...
from decile_core.models import Backtest
...
def _signature(settings: Settings, public_id: str, artefact: str, expires: int) -> str:
    ...
    key = settings.jwt_secret.encode("utf-8") or b"decile-unconfigured"
    return (
...
def _csv(header: Sequence[str], rows: Sequence[Sequence[object]]) -> bytes:
    """Minimal RFC 4180, ``\\n`` line endings — the same shape as ``decile_api.csv_export``."""
    lines = [",".join(header)]
