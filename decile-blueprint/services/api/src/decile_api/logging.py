...
``local`` keeps plain text (``DECILE_LOG_JSON=false``), because a developer reads it directly.
...
It is a backstop, not the design. The design is that `decile_api.security` and the auth service
...
request_id_var: ContextVar[str | None] = ContextVar("decile_request_id", default=None)
