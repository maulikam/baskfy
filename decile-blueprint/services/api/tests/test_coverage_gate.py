...
def _files(core: tuple[int, int], api: tuple[int, int]) -> Files:
    return {
        "packages/core/src/baskfy_core/factors.py": _file(*core),
        "services/api/src/baskfy_api/app.py": _file(*api),
        "packages/providers/src/baskfy_providers/nse.py": _file(100, 5),
        "services/worker/src/baskfy_worker/orchestrator.py": _file(100, 40),
...
class TestAttribution:
    def test_a_file_is_attributed_to_its_package(self) -> None:
        assert package_of("packages/core/src/baskfy_core/factors.py") == "baskfy_core"
        assert package_of("/abs/services/api/src/baskfy_api/app.py") == "baskfy_api"
    ...
    def test_the_stdlib_does_not_dilute_a_package(self) -> None:
        ...
        assert summary["baskfy_core"].total == 1000
...
class TestTheArithmetic:
    def test_branches_count_towards_the_percentage(self) -> None:
        ...
        row = PackageCoverage(
            "baskfy_core", statements=100, missing=0, branches=100, partial_branches=50
        )
        assert row.total == 200
        assert row.percent == pytest.approx(75.0)

    def test_an_empty_package_is_not_a_division_by_zero(self) -> None:
        assert PackageCoverage("baskfy_core", 0, 0, 0, 0).percent == 100.0
...
class TestTheGate:
    ...
    def test_it_fails_when_core_is_below_ninety(self) -> None:
        ...
        assert problems[0].startswith("baskfy_core: 85.00% is below the 90%")

    def test_it_fails_when_api_is_below_eighty(self) -> None:
        problems = failures(summarise(_report((1000, 50), (1000, 250))))
        assert len(problems) == 1
        assert problems[0].startswith("baskfy_api: 75.00% is below the 80%")
    ...
    def test_a_package_missing_from_the_report_is_a_failure_not_a_pass(self) -> None:
        ...
        report: dict[str, object] = {
            "files": {"packages/core/src/baskfy_core/factors.py": _file(100, 0)}
        }
        problems = failures(summarise(report))
        assert problems == ["baskfy_api: not present in the coverage report (did its tests run?)"]

    def test_the_ungated_packages_are_reported_but_never_fail_the_build(self) -> None:
        files = _files((1000, 50), (1000, 100))
        files["services/worker/src/baskfy_worker/orchestrator.py"] = _file(1000, 900)
        summary = summarise({"files": files})
        assert summary["baskfy_worker"].percent < 80
...
class TestTheCommand:
    def test_it_refuses_to_run_without_a_database(
        ...
        """Without one the db-marked tests skip and baskfy_api is not the number CI gates."""
        monkeypatch.delenv("BASKFY_TEST_DATABASE_URL", raising=False)
    ...
    def test_it_passes_with_a_database_and_a_healthy_report(
        ...
        monkeypatch.setenv("BASKFY_TEST_DATABASE_URL", "postgresql+asyncpg://x/y")
        report = tmp_path / "coverage.json"
        report.write_text(json.dumps(_report((1000, 50), (1000, 100))))
    ...
    def test_the_thresholds_are_the_ones_prompt_19_names(self) -> None:
        assert THRESHOLDS == {"baskfy_core": 90.0, "baskfy_api": 80.0}
