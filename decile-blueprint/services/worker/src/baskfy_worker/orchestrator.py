...
async def run_compute_vbt_step(
    ...
    """Step 13 (VB4): the volume-breakout detector, over the bars this chain has just published.

    **This coroutine cannot raise, and that is its whole job**, for the reason
    :func:`run_compute_swing_step` above it cannot: a `vb_signal_daily` row nobody wrote is a page
    saying "no candidates today", while a nightly run that failed is a screener serving yesterday
    to everybody. `docs/vbt/06-module-plan.md` requires the step to be "unable to fail the run"
    and `docs/vbt/DECISIONS-VB.md` VB0.5 records it as a decision rather than an edit, because
    `steps.py` asks for one.

    A step of its own rather than a block inside :func:`_run_chain`, so the guarantee can be
    tested for what it is — "this raises nothing, whatever the detector does" — rather than only
    through a full pipeline run, where the answer would depend on twelve earlier steps.

    ``deps.vbt_user_id`` is ``None`` on a deployment with no sole tenant, and then the step says
    so and does nothing. ``deps.vbt_nightly_enabled`` is `docs/vbt/02` Track B's one on-by-default
    flag: detection moves no money, and a sleeve with no history is a sleeve with no evidence.
    """
    async with record_step(session, run_id, PipelineStep.COMPUTE_VBT, trade_date) as step:
        if not deps.vbt_nightly_enabled:
            step.status = StepStatus.SKIPPED
            step.note(skipped_reason="BASKFY_VBT_NIGHTLY_ENABLED is false")
            return
        ...
        try:
            ...
            step.status = StepStatus.SKIPPED
            step.note(skipped_reason=f"{type(exc).__name__}: {exc}")
            log.warning("compute_vbt failed for %s: %s", trade_date, exc)
