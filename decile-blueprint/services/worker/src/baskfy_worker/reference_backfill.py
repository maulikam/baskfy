"""Backfill the reference data over a historical range (Prompt 4 deliverable 5).
    ...
    python -m baskfy_worker.reference_backfill --from 2018-01-01 --to 2026-08-18
...
seconds of work, unlike the multi-year bar chunks ``baskfy_worker.backfill`` manages.
"""
...
from baskfy_worker.tasks.listings import run_refresh_listings
from baskfy_worker.tasks.market_health import run_compute_market_health
from baskfy_worker.tasks.membership import run_refresh_index_membership
from baskfy_worker.tasks.snapshots import run_refresh_index_snapshots
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="python -m baskfy_worker.reference_backfill",
        description=__doc__,
