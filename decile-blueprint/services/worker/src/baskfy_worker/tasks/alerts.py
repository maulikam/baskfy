...
The work itself is ``baskfy_api.alerts.dispatch`` and ``baskfy_api.webhooks``; what is here is the
two ``run_*`` coroutines the Celery bindings wrap, in the same shape as every pipeline step
(``baskfy_worker.tasks.*``): a coroutine that takes a session and returns a JSON-able summary, so
...
``baskfy.pipeline.nightly`` ends there. An alert that failed to send must not roll back a publish,
