...
The arithmetic lives in ``baskfy_core.factors`` (pure, per docs/02's I/O-free rule for
...
from baskfy_core.factors import TOP_RISK_FLAG_PERCENTILE, FactorResult
from baskfy_core.models import FactorDaily, IndexMemberDaily, TradingDay
...
class FactorEngine(Protocol):
    ...
    ``baskfy_worker.engine`` is the implementation; the protocol stays so a recalibrated engine
...
async def universe_masks(session: AsyncSession, on: dt.date) -> dict[int, int]:
    ...
    for instrument_id, index_id in rows.tuples():
        ...
        if universe is None:
            ...
            # universe range are never masked. See baskfy_core.universes.
            continue
