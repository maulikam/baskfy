...
for the same metrics ``decile_api.metrics`` exposes. The publish deadline is in **both**: the
...
Sinks
-----
An alert always increments :data:`decile_api.metrics.ALERTS` and always logs a structured record
at ERROR with ``alert=`` set, which is what a Loki alert rule matches on. Beyond that it goes to
whichever sinks are configured: Sentry when a DSN is set, email when ``DECILE_OPS_ALERT_EMAIL`` is,
and a JSON webhook when ``DECILE_OPS_ALERT_WEBHOOK_URL`` is. **None configured is a valid
...
from decile_api import sentry
from decile_api.email import Mailer, Message, build_transport
from decile_api.metrics import ALERTS
...
async def dispatch(
    ...
    Async because the mail transport is (``decile_api.email.Mailer.deliver``), and every caller —
    ...
    annotate_current_span({"decile.alert": alert.name.value})
    log.error(
        "alert",
        extra={
...
def webhook_payload(alert: Alert) -> JsonObject:
    """The body posted to ``DECILE_OPS_ALERT_WEBHOOK_URL``.
...
async def _send_email(alert: Alert, settings: Settings, mailer: Mailer | None) -> bool:
    ...
    Deliberately not one of the rendered templates in ``decile_api.email.templates``: those are
...
def _warn_if_unheard(delivered: list[str]) -> None:
    ...
    log.warning(
        "alerts have no delivery sink configured; set DECILE_SENTRY_DSN, "
        "DECILE_OPS_ALERT_EMAIL or DECILE_OPS_ALERT_WEBHOOK_URL"
    )
