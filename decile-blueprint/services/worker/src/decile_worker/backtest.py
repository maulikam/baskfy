...
``decile_core.backtest`` is pure and knows nothing about PostgreSQL (docs/02 §"Repo layout").
...
and `factor_daily` at `d`." ``decile_core.screener`` already builds exactly that statement for a
...
in the frame or not (``decile_core.rebalance`` exits it as ``rank_outside_buffer`` if it is there
...
from decile_api.screener import current_data_version, execute_screen
...
async def load_backtest_data(
    ...
    *,
    ...
    """Assemble everything :func:`decile_core.backtest.run_backtest` reads.
