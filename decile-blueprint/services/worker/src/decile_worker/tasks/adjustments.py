...
docs/09 §"Adjustment algorithm" gives the maths, which lives in ``decile_core.adjustments``
...
from decile_core.models import CorporateAction, OhlcvDaily
...
def _issue_price(raw: object) -> Decimal | None:
    ...
    ``decile_core.adjustments`` refuses to adjust a rights issue without it rather than guessing;
