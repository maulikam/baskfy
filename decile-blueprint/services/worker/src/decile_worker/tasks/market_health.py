...
--------------------------
Not here. ``decile_core.breadth.breadth_query`` builds the statement, and this step executes and
stores it — the same split as ``decile_core.screener`` versus ``decile_api.screener``, and for the
...
history queryable — ``decile_api.market_data.market_health_history`` is the reader, and it joins
...
from decile_core.breadth import ATH_PROXIMITY_PCT, BREADTH_COLUMNS, breadth_query
from decile_core.models import MarketHealthDaily
