...
their ids are load-bearing — see ``decile_core.universes``) while the dashboard grows to whatever
...
from decile_core.models import IndexDef, IndexSnapshotDaily
from decile_core.universes import FIRST_NON_UNIVERSE_INDEX_ID, UNIVERSES, slugify_index
from decile_providers.errors import ProviderError
from decile_providers.records import IndexSnapshot
...
async def _next_dashboard_id(session: AsyncSession) -> int:
    ...
    (``decile_core.universes``). A dashboard index that stole a bit would silently change which
