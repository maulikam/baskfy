...
editable, src-layout packages**: ``decile_core`` resolves through
``.venv/.../_editable_impl_decile_core.pth``, which is an absolute path to the *real*
...
``decile_core`` to a temporary directory **once**, mutates one file in the copy per mutant, and
...
#: ``decile_core.reference_export.default_fixture_path`` finds the committed CSV by walking
