...
| M11 | 271-row parity test | 🟡 **measured for the first time, 31 Aug** | 31 Aug 2026 | ⚠️ The 22 Aug figure **had never been executed** — the two full-row cases skip on an unset `BASKFY_PARITY_BARS`, so a green suite said nothing about parity. Run with real bars (223,997 rows): **6,396 of 9,186**, now **4,514** after the half-open window fix. Window length was only 1,882 of them; five further causes diagnosed in `docs/PARITY-M11.md`. RSI (Cutler@N−1) and volatility (ddof=0) authorised by Maulik 31 Aug, landing as leaf 2.1 |
| M12 | Desk parity | ✅ **green, 31 Aug** | 31 Aug 2026 | **25/25 top-25** (was 23/25), top-15 15/15, max \|ΔSCORE\| **4.30 pts** against `REPLACEMENT_EDGE` 8.0 — no swap decision can differ on that corpus. The gap was missing corporate actions and M28 closed it |
| M13 | MomentumScan — CSV cord cut | ⚠️ **built, not switched on — for a NEW reason** | 31 Aug 2026 | ⚠️ "223 vs 239" is **stale and was keeping M13 blocked for a fixed problem**. The gap is now **0** (239 vs 239; ledger 16 → 2 → 0: M24/M27/M28 backfills, then M11's window fix). **The real blocker, previously unrecorded:** `_carried_columns()` (`app/main.py:378`) sources marketcap/beta/circuits/F&O from the most recent **uploaded** CSV and `momentum_scan.build` inner-joins on symbol — so the generated scan's universe is *defined by* an upload. It does not cut the cord; it requires one. `docs/PARITY-M13.md` |
| M14 | Breadth + shadow harness | ⚠️ **built; shadow amber** | 31 Aug 2026 | Re-run 31 Aug: **2 deltas at HEAD, 3 with the window fix** (was 4), all quantity-only on names both sides buy, reference prices identical to the paisa. **The corporate-action substitution is CLOSED** — SHILPAMED 2:1, ex 2025-10-03, recovered by M24 and applied by M28. Remaining: WELCORP, an RSI-definition disagreement across the >78 cliff. **The harness has never generated a sell** (`BOOK = []`), so every exit and retention rule is untested. Flag off. `docs/PARITY-M14.md` |
