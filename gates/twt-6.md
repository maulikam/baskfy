...
- [ ] G1: The evening plan writes `tw_plan` (source `EVENING`) with its lines and skips, and the
  ...
  CHECK: cd decile-blueprint && uv run pytest -q -k "twt and (evening or morning) and plan" 2>&1 | tail -3
  EXPECT: /passed/
  EVIDENCE: pending
...
- [ ] G2: `POST /twt/execute` requires `confirm=true` and a `plan_id`; **an expired plan is 410 and
  ...
  CHECK: cd decile-blueprint && uv run pytest -q -k "twt and (expired or confirm) and execute" 2>&1 | tail -3
  EXPECT: /passed/
  EVIDENCE: pending
...
- [ ] G3: **A stop never falls.** A `RAISE_GTT_STOP` at or below the resting trigger is `BLOCKED`,
  ...
  CHECK: cd decile-blueprint && uv run pytest -q -k "twt and (never_falls or stop_below or above_last)" 2>&1 | tail -3
  EXPECT: /passed/
  EVIDENCE: pending
...
- [ ] G4: **The delete-and-replace path's two failure halves.** A cancel that fails leaves the OLD
  ...
  CHECK: cd decile-blueprint && uv run pytest -q -k "twt and (cancel_fails or arm_fails or naked)" 2>&1 | tail -3
  EXPECT: /passed/
  EVIDENCE: pending
...
- [ ] G5: **Every fill arms a GTT in the same request** (non-negotiable 4), and a fourth
  ...
  CHECK: cd decile-blueprint && uv run pytest -q -k "twt and (arms_gtt or session_cap)" 2>&1 | tail -3
  EXPECT: /passed/
  EVIDENCE: pending
...
- [ ] G6: A second confirm of the same line is refused by the idempotency key
  ...
  CHECK: cd decile-blueprint && uv run pytest -q -k "twt and (idempot or double_send or client_id)" 2>&1 | tail -3
  EXPECT: /passed/
  EVIDENCE: pending
...
- [ ] G7: **With the flag false, `place` is called only on the dry-run adapter — 0 orders reach a
  ...
  CHECK: cd decile-blueprint && uv run pytest -q -k "twt and (dry_run or no_broker or spy)" 2>&1 | tail -3
  EXPECT: /passed/
  EVIDENCE: pending
...
- [ ] G8: **TW6a's six routes exist and are tested**, and `/twt/halt`'s four behaviours are each
  ...
  CHECK: cd decile-blueprint && uv run pytest -q -k "twt and (halt or reconcile or rearm or sweep_route)" 2>&1 | tail -3
  EXPECT: /passed/
  EVIDENCE: pending
...
- [ ] G9: Every POST to the desk carries an `Origin` header and one without it is refused —
  ...
  CHECK: cd decile-blueprint && uv run pytest -q -k "twt and origin" 2>&1 | tail -3
  EXPECT: /passed/
  EVIDENCE: pending
