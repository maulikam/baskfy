...
- [ ] G1: In the backtest — a fill whose session low breaches the initial stop closes **that
  ...
  CHECK: cd decile-blueprint && uv run pytest packages/core/tests/test_twt_fill_day.py -q 2>&1 | tail -3
  EXPECT: /passed/
  EVIDENCE: pending
...
- [ ] G2: The three boundary cases, because this is where an off-by-one costs money: an **open
  ...
  CHECK: cd decile-blueprint && uv run pytest packages/core/tests/test_twt_fill_day.py -q -k "open_below or exactly or tick" 2>&1 | tail -3
  EXPECT: /passed/
  EVIDENCE: pending
...
- [ ] G3: **The safety property.** Over a GENERATED book of fills and sessions, no session ends
      ...
      spot check.
  CHECK: cd decile-blueprint && uv run pytest -q -k "twt and (safety_propert or no_naked)" 2>&1 | tail -3
  EXPECT: /passed/
  EVIDENCE: pending

- [ ] G4: The sweep is **idempotent and keyed on the day** — running it twice re-arms nothing
      twice and raises nothing twice.
  CHECK: cd decile-blueprint && uv run pytest -q -k "twt and sweep and idempot" 2>&1 | tail -3
  EXPECT: /passed/
  EVIDENCE: pending
...
- [ ] G5: `TWT_POSITION_NAKED` fires on any naked line at any time, and
  ...
  CHECK: cd decile-blueprint && uv run pytest -q -k "twt and (naked_alert or missing_at_1515)" 2>&1 | tail -3
  EXPECT: /passed/
  EVIDENCE: pending
