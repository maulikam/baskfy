...
from decile_core.circuits import (
    DEFAULT_CIRCUIT_CONFIG,
    CircuitConfig,
    circuit_hit_expr,
    circuit_method_expr,
)
from decile_core.momentum import (
...
from decile_core.regime import RegimeConfig, classify_regimes
...
def _with_window_factors(
    ...
    for window in windows.values():
        ...
        # §12: circuit-hit count over the window (INFERRED — see decile_core.circuits).
        expressions.append(
            pl.col("circuit_hit")
            .cast(pl.Float64)
            .rolling_sum(window_size=n, min_samples=n)
...
def _with_skip_month(frame: pl.DataFrame, config: FactorConfig) -> pl.DataFrame:
    """docs/05 §8 — INFERRED. See decile_core.momentum for the two candidate definitions."""
    return frame.with_columns(
