...
``app_user.is_staff``
    ...
    (``decile_core.models.accounts``) rather than here, because a second table joined on every
...
:class:`EntitlementOverride`
    ...
    own row, with an author, a reason and an expiry, and ``decile_api.entitlements`` consults it
...
from decile_core.models.base import Base, BigIntPk, CreatedAt, JsonObject
...
class EntitlementOverride(Base):
    ...
    ``feature`` is a :class:`decile_core.entitlements.Feature` value, or
    :data:`decile_core.entitlements.MAX_SCREENS_KEY` — in which case ``value`` carries the number.
    Keeping both in one table is what lets ``decile_api.entitlements`` apply overrides in a single
...
#: from three places. ``app_user.is_staff`` is declared on :class:`decile_core.models.AppUser`.
