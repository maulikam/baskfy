...
rebuild. Regenerate with ``python -m decile_providers.fixture_builder``.
"""
...
from decile_core.trading_calendar import build_calendar
from decile_core.universes import UNIVERSES, slugify_index
...
#:
#: It is deliberately shorter than 145. `decile_worker.tasks.snapshots` makes the point in
...
def trading_days(end: dt.date, years: int = FIXTURE_YEARS) -> list[dt.date]:
    """The fixture's calendar, from ``decile_core`` so it matches the rest of the system.
...
Generated by `python -m decile_providers.fixture_builder`. Deterministic: seed {FIXTURE_SEED}.
