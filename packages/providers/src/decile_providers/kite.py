...
from decile_providers.ports import BARS_CAPABILITIES, Capability, ProviderHealth
from decile_providers.ratelimit import RateLimiter
...
class KiteProvider:
    ...
    def check(self) -> ProviderHealth:
        ...
        if not self._settings.kite_api_key:
            problems.append("DECILE_KITE_API_KEY is not set")
        if not self._settings.kite_api_secret:
            problems.append("DECILE_KITE_API_SECRET is not set")
        if not self._settings.token_encryption_configured():
            problems.append("DECILE_KITE_TOKEN_ENCRYPTION_KEY is not set")
    ...
    def _authenticated_client(self) -> KiteClientLike:
        ...
        """
        if not self._settings.kite_api_key:
            raise CredentialsMissing("DECILE_KITE_API_KEY is not set", provider=self.name)
