...
The HTTP surface is ``decile_api.routers.billing``; the gateway client and the signature
arithmetic are ``decile_api.razorpay``; this module is what happens to our own rows.
...
from decile_api.razorpay import RazorpayGateway, from_paise
from decile_api.settings import Settings
from decile_core.models import AppUser, Payment, Plan, Subscription, WebhookEvent
...
NOTE_USER: Final = "decile_user_public_id"
NOTE_PLAN: Final = "decile_plan_code"
...
def _gateway_plan_id(settings: Settings, plan: Plan) -> str:
    ...
    if not gateway_id:
        raise CheckoutError(
            ...
            f"DECILE_RAZORPAY_PLAN_ID_{'MONTHLY' if plan.interval == 'month' else 'YEARLY'}."
        )
    return gateway_id
...
class WebhookContext:
    ...
    settings: Settings
    #: Where an invoice PDF is written (`decile_api.invoices.build_invoice_archive`).
    archive: RawArchive
