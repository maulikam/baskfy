...
* ``instrument`` powers the listings register — docs/04 has no ``listing`` table, and
  ``decile_worker.tasks.listings`` explains why there should not be one: a listing *is* an
  instrument.
...
from decile_core.universes import UNIVERSE_BY_SLUG, Universe
...
async def market_health(session: AsyncSession, slug: str, as_of: dt.date) -> MarketHealth:
    ...
    not derived on the fly, and ``decile_worker.tasks.market_health`` is what writes it.
    """
