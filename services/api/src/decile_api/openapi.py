...
Usage:
    python -m decile_api.openapi                 # write packages/api-client/openapi.json
    python -m decile_api.openapi --check         # exit 1 if the checked-in file is stale
    python -m decile_api.openapi --stdout        # print it
...
def render() -> str:
    ...
    document must not change because a developer has a different ``DECILE_ENVIRONMENT``.
    """
