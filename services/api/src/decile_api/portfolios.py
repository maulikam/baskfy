...
The database half of Prompt 14. ``decile_core.rebalance`` owns the *rule* and
``decile_core.portfolio_csv`` owns the *parser*; both are pure. This module is the I/O between
...
from decile_core.rebalance import HeldName, RebalancePlan, ScreenRank, plan_rebalance
...
class PortfolioNotFound(LookupError):
    ...
    ``decile_api.routers.screens`` gives: a 404 that distinguishes them confirms the id exists.
    """
...
async def run_rebalance(  # noqa: PLR0913 - the screen, the date and the two rule inputs are separate
    ...
    *,
    ...
    The screen is executed **uncached**: :func:`decile_api.screener.run_screen` returns the cached
    ...
    string-matching the rule refuses to do (``decile_core.rebalance.plan_rebalance``).
    """
