...
* ``GET /backtests/{id}/download/{artefact}`` — what the signed URL from ``/export`` redeems
  against. See ``decile_api.backtests`` for why the link is signed by this service.
...
from decile_api import backtests as service
from decile_api import idempotency, invoices
from decile_api.auth import AuthenticatedDep, Principal, settings_for
from decile_api.db import SessionDep
from decile_api.entitlements import EntitlementsDep, Feature
from decile_api.problems import Problem, ProblemType, not_found, rate_limited, stale_data_version
...
#: The name of the task the worker binds in ``decile_worker.tasks.celery_tasks``.
BACKTEST_TASK: Final = "decile.backtest.run"
...
async def download_artefact(
    ...
    return Response(
        ...
        headers={
            "Content-Disposition": (
                f'attachment; filename="decile-backtest-{public_id}-{artefact}.csv"'
            ),
