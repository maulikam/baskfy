...
One module, three callers: :func:`decile_api.app.create_app`, the Celery worker's
...
same redaction the logs use (``decile_api.logging.redact_text``) over every event's message and
...
from decile_api.logging import REDACTED, redact_text
...
def configure_sentry(settings: Settings, *, service: str) -> bool:
    ...
    ``service`` names the runtime (``decile-api``, ``decile-worker``) and becomes the
