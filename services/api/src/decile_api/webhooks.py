...
and how a delivery is retried. The rows are ``decile_core.models.integrations``; the schedule that
drives attempts is ``decile_worker.tasks.webhooks``.
...
``DECILE_WEBHOOK_SIGNING_SECRET``, falling back to the JWT secret (which production already
...
from decile_core.models import WebhookDelivery, WebhookEndpoint
...
def master_secret(settings: Settings) -> str:
    ...
    if not master:
        raise WebhookNotConfigured(
            "DECILE_WEBHOOK_SIGNING_SECRET (or DECILE_JWT_SECRET) is empty; webhooks cannot be "
