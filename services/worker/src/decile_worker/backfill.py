...
fixes the Python namespace as ``decile_worker``, so the real entry point is
``python -m decile_worker.backfill``. ``make backfill`` wraps it.
...
from decile_core.models import CURSOR_KIND_BARS, IngestCursor, Instrument
...
from decile_worker.tasks.bars import upsert_bars
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="python -m decile_worker.backfill",
        description=__doc__,
