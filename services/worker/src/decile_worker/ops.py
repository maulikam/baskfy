...
--------------------------------------
``decile_worker.orchestrator`` runs the whole ten-step chain inside **one** transaction, and says
...
from decile_api.metrics import IST
from decile_api.settings import Settings, get_settings
from decile_core.models import PipelineRun, TradingDay
from decile_core.seed_data import NSE_EXCHANGE_ID
from decile_worker.alerts import Alert, AlertName, Severity
from decile_worker.steps import RunStatus
...
async def begin_run(session: AsyncSession, trade_date: dt.date) -> int:
    ...
    :func:`decile_worker.steps.open_run` does, so a re-delivered Celery message does not open a
...
async def fail_run(session: AsyncSession, run_id: int, error: str) -> PipelineRun | None:
    ...
    :class:`~decile_worker.steps.HardFailure` — at that point the chain's own transaction has
...
def _deadline(settings: Settings) -> dt.time:
    """``DECILE_PUBLISH_DEADLINE_IST`` as a time. Falls back to docs/11's 20:15 on a bad value."""
    try:
        hour, minute = (int(part) for part in settings.publish_deadline_ist.split(":", 1))
        return dt.time(hour, minute)
    except (TypeError, ValueError):
        log.warning(
            "DECILE_PUBLISH_DEADLINE_IST is not HH:MM; using docs/11's 20:15",
...
def check_kite_token(
    ...
    expiry is *computed* from the issue time (``decile_providers.tokens``), and a network call to
    ...
    from decile_providers.settings import get_provider_settings  # noqa: PLC0415
...
async def check_queue_backlog(
    ...
    Also carried as a Prometheus rule over ``decile_queue_depth`` (``infra/prometheus/alerts.yml``)
