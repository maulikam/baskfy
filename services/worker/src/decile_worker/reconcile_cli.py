...
:mod:`decile_core.reconcile`. This is what runs in CI and what
...
recomputes every mapped column with :func:`decile_core.factors.compute_factors` and diffs against
...
from decile_core.precision import COLUMN_PRECISION, quantise
...
def recompute_against_bars(
    ...
    :func:`decile_core.factors.compute_factors` requires. The comparison is done **after** storage
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="python -m decile_worker.reconcile_cli", description=__doc__
    )
    parser.add_argument(
        "--export",
        type=Path,
