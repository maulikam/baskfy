...
engine they both stand on is ``decile_api.screener`` (Prompt 6).
"""
...
from decile_core.screener import CACHE_NAMESPACE
from decile_worker.steps import RunStatus, StepOutcome

#: docs/06 §Caching: "Key: `screen:{sha256(...)}:{as_of}:{data_version}`". Purging by prefix is
#: what "invalidate the whole namespace" means in Redis terms. Defined in ``decile_core.screener``
